import sys; sys.path.insert(0,'adaptive_control'); sys.path.insert(0,'.')
import numpy as np
from numpy import sin, cos, pi
from scipy.integrate import solve_ivp
from scipy.linalg import solve_continuous_lyapunov
from dynamics import *

V_supply=12.0; g=9.81
Ia_stall=4.9; tau_stall=220*g/1000
Ia_no_load=0.2; theta_dot_no_load=130*2*pi/60
Kt=tau_stall/Ia_stall; Ra=V_supply/Ia_stall
Bm=Kt*Ia_no_load/theta_dot_no_load
Jm=0.093; r_spindle=0.01
A_true=np.array([[0.,1.],[0.,-Bm/Jm]]); B_true=np.array([[0.],[Kt/Jm]])
d_true=lambda tau: np.array([0., tau/Jm])
omega_n=8.; zeta=1.
A_m=np.array([[0.,1.],[-omega_n**2,-2.*zeta*omega_n]]); B_m=np.array([[0.],[omega_n**2]])
Q=np.diag([10.,1.]); P=solve_continuous_lyapunov(A_m.T,-Q)
Gamma_K=np.diag([20.,3.]); gamma_L=20.
K_0=[0.,0.]; L_0=1.
wt_frac1=2/3; wt_frac2=1/3
force_s1=l1*0.5; force_s2=l2*0.5; force_s3=l3*0.5
r_circle1=0.010; r_circle2=0.010; r_circle3=0.010
aim_frac1=0.5; aim_frac2=0.5; aim_frac3=0.5
_force_s=(force_s1,force_s2,force_s3); _force_r=(r_circle1,r_circle2,r_circle3)
_force_aim=(aim_frac1,aim_frac2,aim_frac3)
r=lambda t: pi/4*(sin(2*pi*0.3*t)>0).astype(float)

def closed_loop_dynamics(t,z):
    theta,omega=z[0],z[1]; xm=z[2:4]; K=z[4:6]; L=z[6]
    phi_1,phi_2,phi_3=z[7:10]; phi_1_dot,phi_2_dot,phi_3_dot=z[10:13]
    x=np.array([theta,omega]); r_t=r(t); e=x-xm
    i_cmd=float(np.clip(-K@x+L*r_t,-Ia_stall,Ia_stall))
    output_force=Kt*i_cmd/r_spindle
    M_mat=M(phi_1,phi_2,phi_3); C_mat=C(phi_1,phi_2,phi_3,phi_1_dot,phi_2_dot,phi_3_dot)
    tau_k=Tau_K(phi_1,phi_2,phi_3); tau_b=Tau_B(phi_1_dot,phi_2_dot,phi_3_dot)
    _force_F=np.array((wt_frac1*output_force,wt_frac2*output_force,0.))
    tau_ext=tau_link_forces(phi_1,phi_2,phi_3,t,z[7:13],_force_s,_force_r,_force_F,_force_aim)
    q_dot=np.array([phi_1_dot,phi_2_dot,phi_3_dot])
    _finger_state=np.array([phi_1,phi_2,phi_3,phi_1_dot,phi_2_dot,phi_3_dot])
    T_cables=cable_tensions(_finger_state,_force_s,_force_r,_force_aim)
    F_motor_from_finger=wt_frac1*T_cables[0]+wt_frac2*T_cables[1]
    tau_finger_on_motor=r_spindle*F_motor_from_finger
    rhs=-C_mat@q_dot-tau_k-tau_b+tau_ext
    q_ddot=np.linalg.solve(M_mat,rhs)
    dtheta=omega
    domega=float(A_true[1,0]*theta+A_true[1,1]*omega+B_true[1,0]*i_cmd+d_true(tau_finger_on_motor)[1])
    dxm=A_m@xm+B_m.flatten()*r_t
    sigma=(B_m.T@P@e.reshape(-1,1)).item()
    dK=Gamma_K@x*sigma; dL=-gamma_L*r_t*sigma
    return np.hstack(([dtheta,domega],dxm,dK,[dL],q_dot,q_ddot))

z0=[0.,0.,0.,0.,K_0[0],K_0[1],L_0,phi1_eq,phi2_eq,phi3_eq,0.,0.,0.]
sol=solve_ivp(closed_loop_dynamics,[0.,10.],z0,t_eval=np.linspace(0,10,5000),method='RK45',rtol=1e-6,atol=1e-8)
print('success:', sol.success)
print('message:', sol.message)
print('steps:', sol.y.shape[1])
print('final phi (deg):', np.degrees(sol.y[7:10,-1]))
print('final theta, omega:', sol.y[0,-1], sol.y[1,-1])
F_all = wt_frac1*np.array([cable_tensions(sol.y[7:13,i],_force_s,_force_r,_force_aim)[0] for i in range(sol.y.shape[1])]) + \
        wt_frac2*np.array([cable_tensions(sol.y[7:13,i],_force_s,_force_r,_force_aim)[1] for i in range(sol.y.shape[1])])
print(f'F_motor_from_finger range: [{F_all.min():.3f}, {F_all.max():.3f}] N')
