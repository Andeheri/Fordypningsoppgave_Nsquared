...
should_use_previously_computed_parameters = True
save_folder = "adaptive_control/figures/with_finger_dynamics"
filename = "mrac_pd_two_motors_long_run" + should_use_previously_computed_parameters * "_precomputed" # Base filename for saved figures and animations
should_save_animation = False  # Set to True to save the animation as a GIF file
...
r_spindle = 0.006  # m (effective radius of the spindle that the cable winds around)
...
# Precomputed values
if should_use_previously_computed_parameters:
    K_f_0 = [29.69983828879948, 5.955480450684962]
    L_f_0 = 30.231230191443682

    K_e_0 = [33.79287646948044, 6.194219636195718]
    L_e_0 = 35.028347381492175
...
sigma_mod_f = 0.1                      # sigma-modification leakage (prevents parameter drift)

Gamma_K_e = np.diag([30.0, 5.0])*0.1   # adaptation rate for K = [K1, K2]
gamma_L_e = 20.0 *0.1                  # adaptation rate for L
sigma_mod_e = 0.1                      # sigma-modification leakage (prevents parameter drift)
...
def closed_loop_dynamics(t, z):
    ...
    # ---- MRAC adaptive laws ----
    if should_use_pure_pd_control or should_use_previously_computed_parameters:
...
def main():
    ...
    z0 = [
        ...
        K_f_0[0], K_f_0[1],          # flexion adaptive K
        L_f_0,                     # flexion adaptive L
        ...
        K_e_0[0], K_e_0[1],          # extension adaptive K
        L_e_0,                     # extension adaptive L
    ...
    # Plot results
    _n_subplots = 4 if should_use_pure_pd_control or should_use_previously_computed_parameters else 5
    ...
    if not should_use_pure_pd_control and not should_use_previously_computed_parameters:
    ...
    _ax_i = 3 if (should_use_pure_pd_control or should_use_previously_computed_parameters) else 4
    ...
    a1_t = phi1_t
    a2_t = phi1_t + phi2_t
    a3_t = phi1_t + phi2_t + phi3_t

    tip_x = (l1 * np.cos(a1_t) + l2 * np.cos(a2_t) + l3 * np.cos(a3_t)) * 1000
    tip_y = (l1 * np.sin(a1_t) + l2 * np.sin(a2_t) + l3 * np.sin(a3_t)) * 1000

    # Equilibrium shadow geometry
    _a1e = phi1_eq
    _a2e = phi1_eq + phi2_eq
    _a3e = phi1_eq + phi2_eq + phi3_eq
    _MC0e = np.array([-l0 * 1000, 0.0])       # proximal end of metacarpal
    _Oe   = np.array([0.0, 0.0])
    _PIPe = _Oe  + l1 * 1000 * np.array([np.cos(_a1e), np.sin(_a1e)])
    _DIPe = _PIPe + l2 * 1000 * np.array([np.cos(_a2e), np.sin(_a2e)])
    _TIPe = _DIPe + l3 * 1000 * np.array([np.cos(_a3e), np.sin(_a3e)])

    import matplotlib.patheffects as _pe
    fig_trace, ax_trace = plt.subplots(figsize=(7, 7))

    # Draw equilibrium finger as faded shadow (metacarpal + phalanges)
    _lw_shadow = 18
    for _p0, _p1, _fc, _oc in [
        (_MC0e, _Oe,   '#8b7355', '#4a3d2e'),   # metacarpal
        (_Oe,   _PIPe, '#24c6c2', '#0f6f6d'),   # PP
        (_PIPe, _DIPe, '#24c6c2', '#0f6f6d'),   # MP
        (_DIPe, _TIPe, '#24c6c2', '#0f6f6d'),   # DP
    ]:
        ax_trace.plot([_p0[0], _p1[0]], [_p0[1], _p1[1]], '-',
                      linewidth=_lw_shadow, solid_capstyle='round',
                      color=_fc, alpha=0.18, zorder=1,
                      path_effects=[_pe.Stroke(linewidth=_lw_shadow + 5,
                                               foreground=_oc, alpha=0.12),
                                    _pe.Normal()])
    for _pt in [_Oe, _PIPe, _DIPe, _TIPe]:
        ax_trace.scatter(_pt[0], _pt[1], s=60, c='#2c3e50',
                         edgecolors='white', linewidths=1.5,
                         alpha=0.2, zorder=2)

    # Fingertip trace, coloured by time
    from matplotlib.collections import LineCollection
    _pts   = np.array([tip_x, tip_y]).T.reshape(-1, 1, 2)
    _segs  = np.concatenate([_pts[:-1], _pts[1:]], axis=1)
    _lc    = LineCollection(_segs, cmap='plasma', linewidth=2.0, zorder=3)
    _lc.set_array(sol.t[:-1])
    ax_trace.add_collection(_lc)
    _cb = fig_trace.colorbar(_lc, ax=ax_trace, location='bottom', pad=0.08)
    _cb.ax.tick_params(labelsize=12)
    _cb.set_label('Time [s]', fontsize=13)

    # Mark start and end
    ax_trace.scatter(tip_x[0],  tip_y[0],  s=80, c='green',  zorder=5,
                     label='Start', edgecolors='white', linewidths=1.2)
    ax_trace.scatter(tip_x[-1], tip_y[-1], s=80, c='red',    zorder=5,
                     label='End',   edgecolors='white', linewidths=1.2)

    ax_trace.set_aspect('equal')
    ax_trace.set_xlabel('x [mm]', fontsize=13)
    ax_trace.set_ylabel('y [mm]', fontsize=13)
    ax_trace.set_title('Fingertip trajectory', fontsize=15)
    ax_trace.tick_params(labelsize=12)
    ax_trace.legend(fontsize=11)
    ax_trace.grid(True, alpha=0.25, linestyle='--')

    plt.tight_layout()
    trace_path = f"{save_folder}/{filename}_fingertip_trace.png"
    plt.savefig(trace_path, dpi=300, bbox_inches='tight')
    print(f"Fingertip trace saved → {trace_path}")
    plt.close(fig_trace)
