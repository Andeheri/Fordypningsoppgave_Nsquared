...
def animate_finger_simulation(sol, l1, l2, l3, speed=1.0, save_fps=None,
                              ...
                              force_scale=None, link_force_mag_ext=None,
                              aim_frac_ext=None, eq_angles=None):
    """
    ...
    link_force_mag    : tuple of three callables F_i(t, s) -> float [N], flexion forces.
    ...
    link_force_mag_ext: tuple of three callables F_i(t, s) -> float [N], extension forces.
                        When provided, extension arrows are drawn at the CW (dorsal)
                        attachment points using a separate colour set.
    aim_frac_ext      : tuple (aim1, aim2, aim3) – fractional target positions for the
                        extension cable. Defaults to aim_frac if not provided.
    eq_angles         : tuple (phi1_eq, phi2_eq, phi3_eq) [rad] – equilibrium joint angles.
                        When provided, a transparent ghost of the finger at that pose is
                        drawn in the background so you can see the deviation from rest.
    """
    if aim_frac_ext is None:
        aim_frac_ext = aim_frac
    save_mode = save_fps is not None
    # ---- Visual style (mirrors visualization_finger_interactive.py) ----
    link_color       = "#24c6c2"
    outline_color    = "#0f6f6d"
    ghost_color      = "#aaaaaa"   # equilibrium ghost
    ghost_alpha      = 0.35
    wrist_color      = "#8b7355"
    ...
    # ---- Ghost finger at equilibrium (drawn first so it sits behind everything) ----
    if eq_angles is not None:
        _eq1, _eq2, _eq3 = eq_angles
        _ea1 = _eq1
        _ea2 = _eq1 + _eq2
        _ea3 = _eq1 + _eq2 + _eq3
        _gMCP = base + L1 * np.array([cos(_ea1), sin(_ea1)])
        _gPIP = _gMCP + L2 * np.array([cos(_ea2), sin(_ea2)])
        _gDIP = _gPIP + L3 * np.array([cos(_ea3), sin(_ea3)])
        for p0, p1 in [(base, _gMCP), (_gMCP, _gPIP), (_gPIP, _gDIP)]:
            ax.plot([p0[0], p1[0]], [p0[1], p1[1]], '-',
                    linewidth=link_width, solid_capstyle='round',
                    color=ghost_color, alpha=ghost_alpha, zorder=0)
        ax.scatter(*_gMCP, s=joint_dot_size, c=ghost_color, alpha=ghost_alpha,
                   edgecolors='none', zorder=0)
        ax.scatter(*_gPIP, s=joint_dot_size, c=ghost_color, alpha=ghost_alpha,
                   edgecolors='none', zorder=0)
        ax.scatter(*_gDIP, s=joint_dot_size, c=ghost_color, alpha=ghost_alpha,
                   edgecolors='none', zorder=0)
    ...
    _lf_colors_ext = ["royalblue", "deepskyblue", "mediumpurple"]
    _lf_labels     = [r"$F_{f,1}$ (flex 1)", r"$F_{f,2}$ (flex 2)", r"$F_{f,3}$ (flex 3)"]
    _lf_labels_ext = [r"$F_{e,1}$ (ext 1)",  r"$F_{e,2}$ (ext 2)",  r"$F_{e,3}$ (ext 3)"]
    _lf_arrow_len = 1.0 * L1   # fixed visual arrow length [mm]
    link_arrow_patches     = [None, None, None]   # flexion arrows, redrawn every frame
    link_arrow_patches_ext = [None, None, None]   # extension arrows, redrawn every frame
    lf_att_scatters    = []   # flexion attachment diamonds
    lf_att_scatters_ext= []   # extension attachment diamonds
    lf_circle_patches  = []   # circles at link midpoints
    lf_aim_scatters     = []   # flexion target dots
    lf_aim_scatters_ext = []   # extension target dots

    def _target_points_mm(a1, a2, a3):
        """Flexion aim-target points in mm (aim_frac, palmar cable)."""
        aim_back_mm = 8.0   # perpendicular offset [mm] toward dorsal side
        MCP_loc = base + L1 * np.array([cos(a1), sin(a1)])
        perp_wrist = np.array([0.0, -1.0])
        perp1      = np.array([sin(a1), -cos(a1)])
        perp2      = np.array([sin(a2), -cos(a2)])
    ...
    def _target_points_ext_mm(a1, a2, a3):
        """Extension aim-target points in mm: target1 static below base;
        target2/3 offset dorsally (down relative to proximal link) from each joint."""
        MCP_loc = base + L1 * np.array([cos(a1), sin(a1)])
        PIP_loc = MCP_loc + L2 * np.array([cos(a2), sin(a2)])
        _d = 10.0   # mm dorsal offset; dorsal direction = [+sin, -cos]
        tgt1 = base    + np.array([0.0, -_d])
        tgt2 = MCP_loc + _d * np.array([ sin(a1), -cos(a1)])
        tgt3 = PIP_loc + _d * np.array([ sin(a2), -cos(a2)])
        return tgt1, tgt2, tgt3
    ...
    def _att_points_mm(a1, a2, a3):
        """Return the three flexion attachment points (midpoint + 90° CCW offset) in mm."""
    ...
    def _att_points_ext_mm(a1, a2, a3):
        """Return the three extension attachment points (midpoint + 90° CW offset) in mm."""
        mid1, mid2, mid3 = _midpoints_mm(a1, a2, a3)
        if link_force_r is not None:
            att1 = mid1 + link_force_r[0] * 1000.0 * np.array([sin(a1), -cos(a1)])
            att2 = mid2 + link_force_r[1] * 1000.0 * np.array([sin(a2), -cos(a2)])
    ...
    def _force_angles_mm(a1, a2, a3):
        """Return world-frame force angles [rad] for flexion (CCW attachment → target)."""
    ...
    def _force_angles_ext_mm(a1, a2, a3):
        """Return world-frame force angles [rad] for extension (CW attachment → ext target)."""
        att1, att2, att3 = _att_points_ext_mm(a1, a2, a3)
        tgt1, tgt2, tgt3 = _target_points_ext_mm(a1, a2, a3)
        phi1 = np.arctan2(tgt1[1] - att1[1], tgt1[0] - att1[0])
        phi2 = np.arctan2(tgt2[1] - att2[1], tgt2[0] - att2[0])
        phi3 = np.arctan2(tgt3[1] - att3[1], tgt3[0] - att3[0])
        return phi1, phi2, phi3
    ...
    if link_force_s is not None:
        ...
        # Extension arrows (CW attachment points)
        if link_force_mag_ext is not None:
            atts_e_i   = _att_points_ext_mm(a1, a2, a3)
            angles_e_i = _force_angles_ext_mm(a1, a2, a3)
            for idx in range(3):
                patch = _make_lf_arrow(atts_e_i[idx], angles_e_i[idx],
                                       link_force_mag_ext[idx](t0, state0_vis),
                                       _lf_colors_ext[idx])
                link_arrow_patches_ext[idx] = patch
                if patch is not None:
                    ax.add_patch(patch)
                sc = ax.scatter(*atts_e_i[idx], s=40, c=_lf_colors_ext[idx], marker='D',
                                edgecolors='black', linewidths=0.8, zorder=7)
                lf_att_scatters_ext.append(sc)

        # Flexion aim-target dots
        ...
        # Extension aim-target dots
        if link_force_mag_ext is not None:
            tgts_e_i = _target_points_ext_mm(a1, a2, a3)
            for tgt, color in zip(tgts_e_i, _lf_colors_ext):
                sc = ax.scatter(*tgt, s=60, c=color, marker='o',
                                edgecolors='black', linewidths=0.8, zorder=7)
                lf_aim_scatters_ext.append(sc)
        ...
        if link_force_mag_ext is not None:
            for color, label in zip(_lf_colors_ext, _lf_labels_ext):
                ax.plot([], [], color=color, lw=2, linestyle='--', label=label)
    ...
    def update(frame):
        ...
        if link_force_s is not None:
            ...
            for idx in range(3):
                ...
                info_lines.append(f"Ff{idx+1}={mag_val:6.1f} N")

            # Extension arrows
            if link_force_mag_ext is not None:
                atts_e   = list(_att_points_ext_mm(a1, a2, a3))
                phis_e   = list(_force_angles_ext_mm(a1, a2, a3))
                for idx in range(3):
                    if link_arrow_patches_ext[idx] is not None:
                        link_arrow_patches_ext[idx].remove()
                    mag_e = link_force_mag_ext[idx](sim_t, state_frame)
                    patch_e = _make_lf_arrow(atts_e[idx], phis_e[idx], mag_e, _lf_colors_ext[idx])
                    link_arrow_patches_ext[idx] = patch_e
                    if patch_e is not None:
                        ax.add_patch(patch_e)
                    lf_att_scatters_ext[idx].set_offsets([atts_e[idx]])
                    info_lines.append(f"Fe{idx+1}={mag_e:6.1f} N")

            force_info_text.set_text("\n".join(info_lines))

            # Update flexion aim-target dots
            if aim_frac is not None:
                tgts = _target_points_mm(a1, a2, a3)
                for sc, tgt in zip(lf_aim_scatters, tgts):
                    sc.set_offsets([tgt])
            # Update extension aim-target dots
            if link_force_mag_ext is not None:
                tgts_e = _target_points_ext_mm(a1, a2, a3)
                for sc, tgt in zip(lf_aim_scatters_ext, tgts_e):
                    sc.set_offsets([tgt])
        ...
        return (fill_wrist, fill_pp, fill_ip, fill_dp,
                ...
                *lf_att_scatters_ext, *lf_circle_patches,
                *lf_aim_scatters, *lf_aim_scatters_ext)
